...
export function AuctionBoardClient({
  ...
  const renderParticipant = ({ user, allTeams, remaining }: typeof participantData[0]) => {
    ...
    return (
      <div
        key={user.id}
        className="flex flex-col min-h-0 border-r border-zinc-800/40 last:border-r-0"
      >
        {/* Name + Balance stacked */}
        <div className="px-3 py-1.5 bg-zinc-900/80 border-b border-zinc-800/40 flex-shrink-0 text-center">
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black flex flex-col">
      {[row1, row2, row3].map((row, i) => (
        <div
          key={i}
          className={`flex-1 grid min-h-0 ${i < 2 ? 'border-b border-zinc-700' : ''}`}
